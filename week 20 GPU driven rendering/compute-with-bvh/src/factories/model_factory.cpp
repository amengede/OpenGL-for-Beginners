...
void MeshFactory::consume(int i) {
    ...
    for (float attribute : vertices) {
        vertexLump.push_back(float_to_half(attribute));
    }
...
Mesh MeshFactory::finalize() {
    ...
    glBufferData(GL_ARRAY_BUFFER, vertexLump.size() * sizeof(uint16_t),
        vertexLump.data(), GL_STATIC_DRAW);
    //position
    glVertexAttribPointer(0, 3, GL_HALF_FLOAT, GL_FALSE, 16, (void*)0);
    glEnableVertexAttribArray(0);
    //texture coordinates
    glVertexAttribPointer(1, 2, GL_HALF_FLOAT, GL_FALSE, 16, (void*)6);
    glEnableVertexAttribArray(1);
    //normal
    glVertexAttribPointer(2, 3, GL_HALF_FLOAT, GL_FALSE, 16, (void*)10);
