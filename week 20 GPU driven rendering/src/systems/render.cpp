...
RenderSystem::RenderSystem(uint32_t shader, uint32_t computeShader, GLFWwindow* window) {
    ...
    frameTimeLocation = glGetUniformLocation(computeShader, "frameTime");
...
void RenderSystem::parse_scene(float frameTime) {

    glUseProgram(computeShader);
    glUniform1f(frameTimeLocation, frameTime);
    glDispatchCompute((maxObjectCount + 63) / 64, 1, 1);
    glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_ATOMIC_COUNTER_BARRIER_BIT | GL_COMMAND_BARRIER_BIT);
}
