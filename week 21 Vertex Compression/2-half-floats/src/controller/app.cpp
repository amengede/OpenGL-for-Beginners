...
void App::make_objects() {
	...
	for (uint32_t i = 0; i < maxObjectCount; ++i) {
		gameObjects.push_back(factory.make_object());
	}

	renderSystem->make_resources(factory.objectCounts, gameObjects);
...
void App::run() {
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		cameraSystem->update();
		renderSystem->update(world->update(frameTime, cameraSystem));
...
void App::make_systems() {
	cameraSystem = new CameraSystem(shader, computeShader);
	world = new World(computeShader);
