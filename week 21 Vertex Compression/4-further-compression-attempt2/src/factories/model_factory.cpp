...
void MeshFactory::read_face(std::vector<std::string> words,
    ...
    std::unordered_map<std::string, uint32_t>& history,
    std::vector<uint32_t>& indices, float magnitude) {

    size_t triangleCount = words.size() - 3;

    for (size_t i = 0; i < triangleCount; ++i) {
        read_corner(words[1], v, vt, vn, vertices, history, indices, magnitude);
        read_corner(words[2 + i], v, vt, vn, vertices, history, indices, magnitude);
        read_corner(words[3 + i], v, vt, vn, vertices, history, indices, magnitude);
    }
