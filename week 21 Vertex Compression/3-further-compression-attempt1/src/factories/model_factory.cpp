...
void MeshFactory::consume(int i) {
    ...
    vertexCount = static_cast<uint32_t>(vertexLump.size() / 9);
...
void MeshFactory::read_corner(std::string description,
    ...
    if (!history.contains(description)) {
        ...
        vertices.push_back(float_to_byte(pos[0]));
        vertices.push_back(float_to_byte(pos[1]));
        vertices.push_back(float_to_byte(pos[2]));
