...
void RenderSystem::batch() {
    ...
    glBufferStorage(GL_SHADER_STORAGE_BUFFER, objectOffsets.size() * sizeof(uint32_t), objectOffsets.data(), GL_DYNAMIC_STORAGE_BIT);
...
void RenderSystem::update() {
    ...
    for (size_t i = 0; i < componentRegistry.renderables.entities.size(); ++i) {
        ...
        if (!cameraSystem->visible(mesh.box, model)) {
            continue;
        }
    ...
    //Draw batches
    for (size_t i = 0; i < objectTypeCount; ++i) {
        Mesh& mesh = meshes[static_cast<ObjectType>(i)];

        glBindVertexArray(mesh.VAO);
        glUniform1i(objectTypeLocation, i);
        glad_glDrawElementsInstanced(GL_TRIANGLES, mesh.elementCount, GL_UNSIGNED_INT, (void*)0, instanceCounts[i]);
    }
